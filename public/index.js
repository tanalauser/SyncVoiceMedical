...
// Add this to your index.js file, replacing the existing language detection logic
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Detect language using the enhanced module
    let currentLanguage;
    
    if (typeof LanguageDetection !== 'undefined' && LanguageDetection.detectLanguage) {
        // Use the enhanced language detection with country priority
        currentLanguage = await LanguageDetection.detectLanguage({
            prioritizeCountry: true,    // This ensures country detection comes first
            defaultLanguage: 'fr'       // Your default if all detection fails
        });
        
        console.log('Language detected by module:', currentLanguage);
    } else {
        // Fallback if module not loaded
        console.error('LanguageDetection module not found!');
        currentLanguage = 'fr';
    }
    ...
    if (elements.languageSelect) {
        ...
        elements.languageSelect.addEventListener('change', function() {
            ...
            // Use the module to switch language (handles storage and URL)
            if (typeof LanguageDetection !== 'undefined') {
                LanguageDetection.switchLanguage(newLang);
            }
            
            // Reload page to apply language change completely
            // Optional: You can remove this if you want seamless switching
            // window.location.reload();
        });
    }

    // Fix login button to use current language
    if (elements.loginBtn) {
        elements.loginBtn.addEventListener('click', function(e) {
            e.preventDefault();
            
            // Get current language from the module
            const currentLang = LanguageDetection ? 
                LanguageDetection.getCurrentLanguage() : 
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (elements.loginBtn) {
        elements.loginBtn.addEventListener('click', function(e) {// index.js - Complete version with enhanced language detection
document.addEventListener('DOMContentLoaded', async function() {
    ...
    async function detectLanguage() {
        ...
        // Check if the enhanced LanguageDetection module is available
        if (typeof LanguageDetection !== 'undefined' && LanguageDetection.detectLanguage) {
            console.log('✅ Using enhanced LanguageDetection module');
            
            try {
                // Use the enhanced module with country priority
                const detectedLang = await LanguageDetection.detectLanguage({
                    prioritizeCountry: true,
                    defaultLanguage: 'fr'
                });
                
                console.log('🎯 Language detected by module:', detectedLang);
                return detectedLang;
            } catch (error) {
                console.error('❌ LanguageDetection module error:', error);
                console.log('⚠️ Falling back to built-in detection');
            }
        } else {
            console.log('⚠️ LanguageDetection module not found, using built-in detection');
        }

        // Built-in fallback detection logic
        let detectedLanguage = null;
        const detectionSteps = [];
    ...
    if (elements.languageSelect) {
        elements.languageSelect.addEventListener('change', function() {
            ...
            // If enhanced module is available, use it to switch language
            if (typeof LanguageDetection !== 'undefined' && LanguageDetection.switchLanguage) {
                LanguageDetection.switchLanguage(newLang);
                console.log('✅ Language switched using LanguageDetection module');
            }
        });
    }
    ...
    if (elements.loginBtn) {
        elements.loginBtn.addEventListener('click', function(e) {
            ...
            let currentLang = detectedLanguage;
            
            // If enhanced module available, use it
            if (typeof LanguageDetection !== 'undefined' && LanguageDetection.getCurrentLanguage) {
                currentLang = LanguageDetection.getCurrentLanguage();
            } else if (elements.languageSelect) {
    ...
    console.log('📊 Final State:', {
        ...
        moduleAvailable: typeof LanguageDetection !== 'undefined'
    });
});
            e.preventDefault();
